...
def build_toy_dataset(N, w, p=0):
  ...
  p: float (optional p=0.0), Percentage of outliers.
  """
  N_out = int(np.floor(N * p))
  N_cor = N - N_out
...
X_train, y_train = build_toy_dataset(N_DATA, w_true, p=P_OUTLIERS)
X_test, y_test = build_toy_dataset(N_DATA, w_true, p=0.0)

# MODEL
X = tf.placeholder(tf.float32, [N_DATA, D])
w = Normal(loc=tf.zeros(D), scale=tf.ones(D))
b = Normal(loc=tf.zeros(1), scale=tf.ones(1))
y = Normal(loc=ed.dot(X, w) + b, scale=tf.ones(N_DATA))
...
for exp in range(N_EXP):
  ...
  visualise(X_train, y_train, qw, qb, n_samples=100, save=name_fig)
